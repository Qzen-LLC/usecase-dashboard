...
export const PATCH = withAuth(async (
  ...
  try {
    ...
    const updatedAssessment = await prismaClient.iso27001Assessment.update({
      where: { id: assessmentId },
      data: {
        progress,
    ...
    console.log('[CRUD_LOG] ISO 27001 Assessment progress updated:', { id: assessmentId, progress, updatedAt: updatedAssessment.updatedAt, authoredBy: userRecord.id });

    return NextResponse.json(updatedAssessment);
  } catch (error) {
    console.error('Error updating ISO 27001 assessment progress:', error);
