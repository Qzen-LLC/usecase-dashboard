...
const calculateDataPrivacyRisk = (qnAData: any) => {
  ...
  console.log("Data Protection:", getAnswer(qnAData, 'RISK_ASSESSMENT', "Data Protection"));
  console.log("Data Volume:", getAnswer(qnAData, 'DATA_READINESS', "Data Volume"));
  console.log("Data Retention Period:", getAnswer(qnAData, 'DATA_READINESS', "Data Retention Period"));
  console.log("Model Update Frequency:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Model Update Frequency"));
  // console.log("Multi-Jurisdiction Handling:", getAnswer(qnAData, 'DATA_READINESS', "Multi-Jurisdiction Handling")); // Question doesn't exist in DB
  const _hasSensitiveData = Array.isArray(dataTypes) && dataTypes.some((type: string) => sensitiveTypes.includes(type || ''));
  ...
  if (Array.isArray(dataTypes) && dataTypes.includes("Children's Data (under 16)")) {
    infoMessages.push("Children's Data (under 16) detected. Data privacy risk has been flagged as sensitive.");
  }
  if (Array.isArray(dataTypes) && dataTypes.includes("Biometric Data")) {
  ...
  if (getAnswer(qnAData, 'DATA_READINESS', "Cross-Border Data Transfer") && Array.isArray(dataProtection) && dataProtection.length === 0) {
  ...
  //TODO
  if (['10TB - 100TB', '> 100TB'].includes(getAnswer(qnAData, 'DATA_READINESS', "Data Volume") ?? '')) {
    score += weights.largeVolume;
    factors.push(`Large data volume >10TB (+${weights.largeVolume})`);
  }

  if (getAnswer(qnAData, 'DATA_READINESS', "Cross-Border Data Transfer") === 'Yes') {
  ...
  // Note: "Multi-Jurisdiction Handling" question doesn't exist in DB - commented out
...
const calculateSecurityRisk = (qnAData: QnAProps[] | null) => {
  let score = 1; // Base score
  const factors = [];
  const infoMessages: string[] = [];
  const weights = {
  ...
  console.log("User Categories:", getAnswer(qnAData, 'BUSINESS_FEASIBILITY', "Target User Categories"));
  console.log("Integration Points:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Integration Points"));
  console.log("Technical Complexity:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Technical Complexity Level"));
  console.log("Deployment Model:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Deployment Model"));
  console.log("Operating Jurisdictions:", getAnswer(qnAData, 'RISK_ASSESSMENT', "Operating Jurisdictions"));

  console.log("CURRENT SECURITY SCORE: ", score);
  const apiSpecs = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "API Specifications");
  const apiSpecsArray = Array.isArray(apiSpecs) ? apiSpecs : (apiSpecs ? [apiSpecs] : []);
  
  if (apiSpecsArray.includes('Public API')) {
    score += weights.publicAPI;
    factors.push(`Public API exposure (+${weights.publicAPI})`);
  } else if (apiSpecsArray.includes('Partner API')) {
    score += weights.partnerAPI;
    factors.push(`Partner API exposure (+${weights.partnerAPI})`);
  } else if (apiSpecsArray.includes('Internal API only')) {
    score += weights.internalAPI;
    factors.push(`Internal API exposure (+${weights.internalAPI})`);
  }
  console.log("AFTER API SPECIFICATIONS: CURRENT SECURITY SCORE: ", score);
  
  // Deployment model risks
  const deploymentModel = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Deployment Model");
  const deploymentArray = Array.isArray(deploymentModel) ? deploymentModel : (deploymentModel ? [deploymentModel] : []);
  
  if (deploymentArray.includes('Public Cloud')) {
    score += weights.cloudDeployment;
    factors.push(`Cloud deployment (+${weights.cloudDeployment})`);
  } else if (deploymentArray.includes('Hybrid Cloud')) {
    score += weights.hybridDeployment;
    factors.push(`Hybrid deployment (+${weights.hybridDeployment})`);
  } else if (deploymentArray.includes('Edge Computing')) {
    score += weights.edgeDeployment;
    factors.push(`Edge deployment (+${weights.edgeDeployment})`);
  }
  console.log("AFTER DEPLOYMENT MODEL: CURRENT SECURITY SCORE: ", score);
  // Integration complexity
  const integrationPoints = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', 'Integration Points') || [];
  const technicalComplexity = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', 'Technical Complexity Level') || 0;
  ...
  console.log("AFTER INTEGRATION COMPLEXITY: CURRENT SECURITY SCORE: ", score); 
  // Authentication complexity (derived from user categories and API exposure)
  const userCategories = getAnswer(qnAData, 'BUSINESS_FEASIBILITY', 'Target User Categories') || [];
  ...
  console.log("AFTER TARGET USER CATEGORIES: CURRENT SECURITY SCORE: ", score);
  
  // Dynamic info: Public/Partner API without security controls
  if (apiSpecsArray.includes("Public API") || apiSpecsArray.includes("Partner API")) {
  ...
  if (deploymentArray.includes("Public Cloud") || deploymentArray.includes("Hybrid Cloud")) {
    if (!integrationPoints || integrationPoints.length === 0) {
...
const calculateRegulatoryRisk = (qnAData: QnAProps[] | null) => {
  let score = 1; // Base score
  const factors = [];
  const weights = {
    GDPR: 3,
  ...
  console.log("Operating Jurisdictions:", getAnswer(qnAData, 'RISK_ASSESSMENT', "Operating Jurisdictions"));
  console.log("Sector-Specific Regulations:", getAnswer(qnAData, 'RISK_ASSESSMENT', "Sector-Specific"));
  console.log("Data Types:", getAnswer(qnAData, 'DATA_READINESS', "Data Types"));
  console.log("Data Protection:", getAnswer(qnAData, 'RISK_ASSESSMENT', "Data Protection"));
  console.log("Explainability Level:", getAnswer(qnAData, 'ETHICAL_IMPACT', "Explainability Level"));
  ...
  let sectorSpecific = getAnswer(qnAData, 'RISK_ASSESSMENT', 'Sector-Specific');
  let sectorSpecificStr = typeof sectorSpecific === 'string' ? sectorSpecific : '';
  if (!sectorSpecificStr && typeof sectorSpecific === 'object' && sectorSpecific !== null) {
  ...
  // GDPR
  let hasGDPR = Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('GDPR (EU)');
  ...
  // HIPAA
  let hasHIPAA = (Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('HIPAA (Healthcare)')) || sectorSpecificStr === 'HIPAA (Healthcare)';
  ...
  let healthcareSector = sectorSpecificStr === 'HIPAA (Healthcare)' || (typeof sectorSpecific === 'object' && sectorSpecific !== null && sectorSpecific['HIPAA (Healthcare)']);
  ...
  // PCI-DSS
  let hasPCI = (Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('PCI-DSS (Payment Cards)')) || sectorSpecificStr === 'PCI-DSS (Payment Cards)';
  let hasFinancialRecords = Array.isArray(dataTypes) && dataTypes.includes('Financial Records');
  ...
  // SOX
  let hasSOX = (Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('SOX (Financial Reporting)')) || sectorSpecificStr === 'SOX (Financial Reporting)' || sectorSpecificStr === 'SOX (Financial Services)';
  ...
  // LGPD (Brazil)
  let hasLGPD = Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('LGPD (Brazil)');
  ...
  // PIPEDA (Canada)
  let hasPIPEDA = Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('PIPEDA (Canada)');
  ...
  // POPI (South Africa)
  let hasPOPI = Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('POPI (South Africa)');
  ...
  // APPI (Japan)
  let hasAPPI = Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('APPI (Japan)');
  ...
  let hasPrivacyActAU = Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('Privacy Act (Australia)');
  ...
  // PDPA (Singapore)
  let hasPDPA = Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.includes('PDPA (Singapore)');
  ...
  if ((hasGDPR || euJurisdictionSelected) && explainabilityLevel === 'Black Box (No Explanation)') {
    score += weights.aiRegulations;
    factors.push(`AI Act compliance (+${weights.aiRegulations})`);
  }

  if (Array.isArray(dataProtectionJurisdictions) && dataProtectionJurisdictions.length > 2) {
    score += weights.multiJurisdiction;
...
const calculateEthicalRisk = (qnAData: QnAProps[] | null) => {
  let score = 1; // Base score
  const factors = [];
  const infoMessages: string[] = [];
  const weights = {
  ...
  console.log("Automation Level:", getAnswer(qnAData, 'ETHICAL_IMPACT', "Decision Automation Level"));
  console.log("Bias Testing:", getAnswer(qnAData, 'ETHICAL_IMPACT', "Bias Testing"));
  console.log("Explainability Level:", getAnswer(qnAData, 'ETHICAL_IMPACT', "Explainability Level"));
  console.log("User Categories:", getAnswer(qnAData, 'BUSINESS_FEASIBILITY', "Target User Categories"));
  console.log("Human Oversight Level:", getAnswer(qnAData, 'ETHICAL_IMPACT', "Human Oversight Level"));
  // Automated decision-making
  const automationLevel = getAnswer(qnAData, 'ETHICAL_IMPACT', 'Decision Automation Level') || '';
  ...
  if (biasTesting === 'No Testing Planned' || biasTesting === 'Basic Statistical Testing') {
  ...
  if (ethicalExplainabilityLevel === 'Black Box (No Explanation)') {
    score += weights.transparencyGap;
    factors.push(`Limited explainability (+${weights.transparencyGap})`);
  }

  // Vulnerable groups
  const ethicalUserCategories = getAnswer(qnAData, 'BUSINESS_FEASIBILITY', 'Target User Categories') || [];
...
const calculateOperationalRisk = (qnAData: QnAProps[] | null) => {
  let score = 1; // Base score
  const factors = [];
  const infoMessages: string[] = [];
  const weights = {
  ...
  console.log("System Criticality:", getAnswer(qnAData, 'BUSINESS_FEASIBILITY', "System Criticality"));
  console.log("Integration Points:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Integration Points"));
  console.log("Model Type:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Model Type"));
  console.log("Failure Impact:", getAnswer(qnAData, 'BUSINESS_FEASIBILITY', "Failure Impact Assessment"));
  console.log("Deployment Model:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Deployment Model"));
  // System criticality
  ...
  const opModelTypes = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', 'Model Type') || [];
  ...
  // Downtime impact
  const failureImpact = getAnswer(qnAData, 'BUSINESS_FEASIBILITY', 'Failure Impact Assessment') || '';
  ...
  if (Array.isArray(opDeploymentModel) && opDeploymentModel.includes('On-Premise')) {
...
const calculateGenAIRisk = (qnAData: QnAProps[] | null) => {
  let score = 1; // Base score
  const factors = [];
  const infoMessages: string[] = [];
  const weights = {
  ...
  console.log("Model Type:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Model Type"));
  console.log("Max Hallucination Rate:", getAnswer(qnAData, 'BUSINESS_FEASIBILITY', "Max Hallucination Rate (%)"));
  console.log("Agent Architecture:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Agent Architecture"));
  console.log("User Categories:", getAnswer(qnAData, 'BUSINESS_FEASIBILITY', "Target User Categories"));
  console.log("Data Types:", getAnswer(qnAData, 'DATA_READINESS', "Data Types"));
  console.log("Model Update Frequency:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Model Update Frequency"));
  console.log("Estimated Monthly Token Usage:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Estimated Monthly Token Usage"));
  console.log("Content Generation Risks:", getAnswer(qnAData, 'ETHICAL_IMPACT', "Content Generation Risks"));
  // Check if Gen AI is being used
  const genAIModelTypes = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', 'Model Type') || [];
  ...
  // Agent autonomy risk
  const agentPattern = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', 'Agent Pattern');
  if (agentPattern && 
      ["Multi-agent Collaborative", "Single agent", "Agent Swarm"].includes(agentPattern)) {
    score += weights.agentAutonomy;
    factors.push(`Complex agent autonomy (+${weights.agentAutonomy})`);
  }

  // Prompt injection vulnerability
  const genAIUserCategories = getAnswer(qnAData, 'BUSINESS_FEASIBILITY', 'Target User Categories') || [];
...
const calculateReputationRisk = (qnAData: QnAProps[] | null) => {
  let score = 1; // Base score
  const factors = [];
  const weights = {
    publicFacing: 2,
  ...
  const repUserCategories = getAnswer(qnAData, 'BUSINESS_FEASIBILITY', 'Target User Categories') || [];
  ...
  if (Array.isArray(repUserCategories) && repUserCategories.includes('Customers') &&
      repSystemCriticality !== 'Non-critical (Experimental)') {
    score += weights.socialMedia;
    factors.push(`Social media amplification (+${weights.socialMedia})`);
  }

  // Trust-critical decisions
  const repSectorSpecific = getAnswer(qnAData, 'RISK_ASSESSMENT', 'Sector-Specific');
  if (Array.isArray(repSectorSpecific) && repSectorSpecific.includes('SOX (Financial Reporting)')) {
  ...
  const repFailureImpact = getAnswer(qnAData, 'BUSINESS_FEASIBILITY', 'Failure Impact Assessment') || '';
...
const RiskCalculation = (qnAData: QnAProps[] | null) => {
  // Check if Gen AI use case
  console.log("Model Type:", getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', "Model Type"));
  const modelTypes = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', 'Model Type') || [];
...
function getMissingAssessmentFields(qnAData: QnAProps[] | null) {
  ...
  const modelTypes = getAnswer(qnAData, 'TECHNICAL_FEASIBILITY', 'Model Type');
  if (!modelTypes || !Array.isArray(modelTypes) || modelTypes.length === 0) missing.push('Technical Feasibility > Model Type');
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {risk.mitigationStrategy && (
                        <div className="mt-2 pt-2 border-t border-neutral-200 dark:border-neutral-700">
                          ...
                          <p className="text-sm text-foreground">{risk.mitigationStrategy}</p>
                        </div>
                      )} */}
                    </div>
                  </div>
